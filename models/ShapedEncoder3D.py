...
class AnisotropicConvSuite(nn.Module):
    def __init__(self, in_ch, out_ch, depth_kernels=(3, 5, 7)):
        ...
        self.depth_convs = nn.ModuleList([
            nn.Conv3d(in_ch, out_ch, kernel_size=(k, 1, 1), padding=(k // 2, 0, 0))
...
class WindowPool3D(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.unfold(2, wd, wd).unfold(3, wh, wh).unfold(4, ww, ww)
...
class AnisotropicSwinBlock(nn.Module):
    def __init__(self, in_ch, out_ch, depth_kernels=(3, 5, 7),
                 window_size=(1, 21, 21), use_attention=True):
        super().__init__()
        self.conv_suite = AnisotropicConvSuite(in_ch, out_ch, depth_kernels)
        ...
        if use_attention:
            ...
            # FIX 4: tokens come from reduced channels, so embed_dim = reduced_ch
            self.attn = KernelMixingAttention(embed_dim=reduced_ch, num_kernels=self.num_kernels)
        else:
    ...
    def forward(self, x, return_weights=False):
        ...
        if self.use_attention:
            tokens = self.window_pool(x_small)       # [B, N, reduced_ch]
            weights = self.attn(tokens)              # [B, N, K]

            wd, wh, ww = self.window_pool._last_window
            Nd, Nh, Nw = D_s // wd, H_s // wh, W_s // ww
            ...
            weights = F.interpolate(weights, size=(D, H, W),
